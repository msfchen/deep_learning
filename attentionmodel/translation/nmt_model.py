...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X = self.model_embeddings.source(source_padded) # source_padded is (src_len, b); X is (src_len, b, e)
        X_packed = pack_padded_sequence(X, source_lengths, batch_first=False)  # return list of batch_sizes of a packed sequence
        X_encoded, (last_hidden, last_cell) = self.encoder(X_packed) # (src_len, b, h*2), (2, b, h), (2, b, h)
        enc_hiddens, _ = pad_packed_sequence(X_encoded, batch_first=False) # inverse op to pack_padded_sequence (src_len, b, h*2)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # enc_hiddens_proj is (b, src_len, h); dec_hidden is (b, h) ==> dec_hidden.unsqueeze(-1) is (b, h, 1) 
        ...
        # goal: (b, 1, src_len) x (b, src_len, 2h) = (b, 1, 2h) => (b, 2h)
        # turn alpha_t from (b, src_len) to (b, 1, src_len)
        a_t = torch.bmm(alpha_t.view((alpha_t.size(0), 1, alpha_t.size(1))), enc_hiddens).squeeze(1)
        U_t = torch.cat((a_t, dec_hidden), dim=1)       # (b, 2h) cat (b, h) ==> (b, 3h)
