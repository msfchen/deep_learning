...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_char_ids = self.words2charindices(sents)                        # list[list[list[int]]]
        sents_padded = pad_sents_char(sents_char_ids, self.char2id['<pad>'])  # list[list[list[int]]]   (batch_size, max_sentence_length, max_word_length)
        sents_tensor = torch.tensor(sents_padded, dtype=torch.long, device=device)     # Constructs a tensor
        return sents_tensor.permute(1, 0, 2)                                                       # (max_sentence_length, batch_size, max_word_length)
