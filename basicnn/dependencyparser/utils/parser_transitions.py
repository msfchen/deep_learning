class PartialParse(object):
    def __init__(self, sentence):
        ...
        ###
        self.stack = ["ROOT"]
        self.buffer = sentence[:]    # create a copy
        self.dependencies = []
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        minibatch_unfinished_parses = unfinished_parses[:batch_size]
        transitions = model.predict(minibatch_unfinished_parses)
        for index, item in enumerate(minibatch_unfinished_parses):
            item.parse_step(transitions[index])
            if len(item.stack)==1 and len(item.buffer)==0:
