...
def naiveSoftmaxLossAndGradient(
    ...
    ### YOUR CODE HERE
    dot_prod = np.dot(outsideVectors, centerWordVec)    # shape = (vocab_size, ) 
    dot_prod_softmax = softmax(dot_prod)                # shape = (vocab_size, )  y_hat_w
    loss_vocab = -1*np.log(dot_prod_softmax)            # shape = (vocab_size, ) 
    y = np.zeros(outsideVectors.shape[0])               # shape = (vocab_size, ) 
    y[outsideWordIdx] = 1.0
    y_hat_minus_y = dot_prod_softmax-y                  # shape = (vocab_size, ) 
    
    loss = loss_vocab[outsideWordIdx]                   # single value    
    gradCenterVec = np.dot(y_hat_minus_y, outsideVectors)    # shape = (embedding_size, )
    gradOutsideVecs = np.dot(y_hat_minus_y.reshape(outsideVectors.shape[0],1), centerWordVec.reshape(1, centerWordVec.shape[0]))    # shape = (vocab_size, embedding_size)
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for w in outsideWords:
        ...
        loss = loss + l
        gradCenterVecs[centerWordIdx] = gradCenterVecs[centerWordIdx] + gc 
